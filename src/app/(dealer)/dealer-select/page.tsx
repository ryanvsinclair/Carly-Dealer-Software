...
export default async function DealerSelectPage() {
  ...
  if (dealerships.length === 1) {
    redirect(`/dealer/${dealerships[0].dealership_id}`);
  }
          ...
          {dealerships.map((dealership: any) => {
            return (
              <Link
                ...
                href={`/dealer/${dealership.dealership_id}`}
                className="block"
              >
                <Card className="p-8 border border-[#E5E5E5] dark:border-[#262626] bg-white dark:bg-[#171717] hover:bg-[#FAFAFA] dark:hover:bg-[#1F1F1F] transition-colors duration-200 cursor-pointer">
                  <div className="flex items-start gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-[16px] font-bold text-[#171717] dark:text-[#FAFAFA] mb-1">
                        {dealership.name}
                      </h2>
                      <p className="text-[14px] text-[#3B82F6] dark:text-[#3B82F6] mb-2 capitalize">
                        {dealership.role.replace('_', ' ')}
                      </p>
